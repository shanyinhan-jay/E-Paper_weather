...
def detect_symbol(code: str) -> str:
    ...
    # const unsigned char gImage_350_M[] = {...}
    # const unsigned char gImage_350[648] = {...}
    m = re.search(r"const\s+unsigned\s+char\s+([A-Za-z_]\w*)\s*\[", content)
    if m:
        return m.group(1)
    # Fallback to preferred naming
...
# Add per-file symbol alias to avoid name collisions with 3636 symbols
for c in codes:
    symbol = detect_symbol(c)
    alias = f"gImage_{c}_M"
    lines += [f'#define {symbol} {alias}', f'#include "7272/{c}.c"', f'#undef {symbol}']
lines += ['']
...
print('icon_m.h generated with', len(codes), 'entries')
