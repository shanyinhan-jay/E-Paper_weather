# 1 "C:\\Users\\Administrator\\AppData\\Local\\Temp\\tmptok5v00q"
#include <Arduino.h>
# 1 "D:/ink/E-Paper_weather/e_weather/e_weather.ino"
# 21 "D:/ink/E-Paper_weather/e_weather/e_weather.ino"
...
static bool parseBssidString(const char* text, uint8_t outBssid[6]);
static void rememberCurrentWiFiAp();
...
void setup();
void loop();
#line 160 "D:/ink/E-Paper_weather/e_weather/e_weather.ino"
bool isDetailedLoggingEnabled() {
...
static bool resolveMqttServer(IPAddress& resolvedIp, bool forceRefresh = false) {
    ...
    if (directIp.fromString(config.mqtt_server)) {
        ...
        if (isDetailedLoggingEnabled()) {
            ...
            Serial.println("MQTT broker uses direct IP");
        }
        return true;
    }
    ...
    if (isDetailedLoggingEnabled()) {
        ...
        Serial.printf("MQTT DNS resolved (%lu ms)\n", dnsElapsed);
    }
    return true;
}
...
static bool parseBssidString(const char* text, uint8_t outBssid[6]) {
    if (text == nullptr || outBssid == nullptr) return false;
    ...
    for (int i = 0; i < 6; ++i) {
        outBssid[i] = static_cast<uint8_t>(parsed[i]);
    }
    return true;
}
...
static void rememberCurrentWiFiAp() {
    ...
    if (currentChannel <= 0 || currentBssid.length() == 0) return;

    bool changed = (config.wifi_last_channel != currentChannel) ||
                   (currentBssid != String(config.wifi_last_bssid));
    if (!changed) return;

    config.wifi_last_channel = currentChannel;
    strlcpy(config.wifi_last_bssid, currentBssid.c_str(), sizeof(config.wifi_last_bssid));
    Serial.printf("WiFi remembered AP updated: channel=%d bssid=%s\n",
    ...
    if (LittleFS.exists("/config.json")) {
      ...
      if (configFile) {
        ...
        if (!error) {
          ...
          config.wifi_last_channel = doc["wifi_last_channel"] | 0;
          strlcpy(config.wifi_last_bssid, doc["wifi_last_bssid"] | "", sizeof(config.wifi_last_bssid));
        }
      }
    } else {
...
void saveConfig() {
  ...
  doc["wifi_last_channel"] = config.wifi_last_channel;
  doc["wifi_last_bssid"] = config.wifi_last_bssid;
...
void displayMessageWithBitmap(String text, const unsigned char* bitmap, int bitmapWidth, int bitmapHeight) {
    ...
    if (BlackImage != NULL) {
        if (isDetailedLoggingEnabled()) Serial.println("Preparing Image");
        ...
        if (isDetailedLoggingEnabled()) Serial.println("U8G2 begin");
        ...
        if (isDetailedLoggingEnabled()) Serial.println("Processing Text");
        ...
        if (isDetailedLoggingEnabled()) Serial.println("Calculating Layout");
        ...
        if (isDetailedLoggingEnabled()) Serial.println("Drawing Lines");
        ...
        if (isDetailedLoggingEnabled()) Serial.println("Displaying");
        epdFlushFrame(false, BlackImage);
        hibernateEPD();


        if (isDetailedLoggingEnabled()) Serial.println("displayMessage done");
    }
}
...
# 1242 "D:/ink/E-Paper_weather/e_weather/e_weather.ino"
...
void hibernateEPD() {
    if (isDetailedLoggingEnabled()) Serial.println("Hibernating EPD...");
    ...
    if (isDetailedLoggingEnabled()) Serial.println("EPD pins isolated.");
}
...
void mqttCallback(char* topic, byte* payload, unsigned int length) {
  ...
  if (strcmp(topic, config.mqtt_unified_topic) == 0) {
      if (isDetailedLoggingEnabled()) Serial.println("Unified message received");
      JsonDocument doc;
  ...
  if (strcmp(topic, config.mqtt_hourly_topic) == 0) {
      ...
      if (isDetailedLoggingEnabled()) {
          ...
          Serial.println("Hourly forecast message received");
      }
      if (isDetailedLoggingEnabled()) Serial.println("Hourly forecast updated");
      ...
      if (!error) {
          ...
          if (isDetailedLoggingEnabled()) {
              ...
              Serial.printf("Hourly forecast ready: %d item(s)\n", count);
          }
...
bool reconnect() {
  ...
  if (!client.connected()) {
    ...
    if (mqttServerIpValid && isDetailedLoggingEnabled()) {
    ...
    if (isDetailedLoggingEnabled()) {
        Serial.printf("MQTT credentials: user=%s password=%s\n",
                      strlen(config.mqtt_user) > 0 ? config.mqtt_user : "(empty)",
                      strlen(config.mqtt_pass) > 0 ? config.mqtt_pass : "(empty)");
    }
...
# 2244 "D:/ink/E-Paper_weather/e_weather/e_weather.ino"
void handleButtonClick() {
    if (!isHourlyForecastEnabled()) {
...
# 2394 "D:/ink/E-Paper_weather/e_weather/e_weather.ino"
  ...
  if (strlen(config.mqtt_server) > 0 && WiFi.status() == WL_CONNECTED) {
      ...
      if (isDetailedLoggingEnabled()) {
          ...
          Serial.println("MQTT initial connect deferred");
      }
...
void loop() {
  ...
  if (isHourlyForecastEnabled() &&
      ...
      bottomForecastView = BOTTOM_VIEW_DAILY;
      hourlyViewActivatedAt = 0;
      if (isDetailedLoggingEnabled()) Serial.println("Hourly view timeout: switched bottom forecast back to daily view");
      displayWeatherDashboard(false);
  }
  ...
  if (client.connected()) {
      if (!timeClient.isTimeSet()) {
          if (millis() - lastNtpRetry > 5000) {
              lastNtpRetry = millis();
              if (isDetailedLoggingEnabled()) Serial.println("NTP not synced, forcing update...");
              if (timeClient.forceUpdate()) {
                   if (isDetailedLoggingEnabled()) Serial.println("NTP Synced (Loop)");
                   ntpRetryCount = 0;
              } else {
                   ntpRetryCount++;
                   if (ntpRetryCount >= 3) {
  ...
  if (config.full_refresh_period > 0) {
      if (millis() - lastFullRefreshTime > (unsigned long)config.full_refresh_period * 60 * 1000) {
          ...
          if (isDetailedLoggingEnabled()) Serial.println("Custom Full Refresh Triggered");
          displayWeatherDashboard(false);
      }
  }
   ...
   if (config.request_interval > 0 && client.connected()) {
       if (millis() - lastRequestTime > (unsigned long)config.request_interval * 60 * 1000) {
          ...
          if (isDetailedLoggingEnabled()) Serial.println("Sending Periodic Battery Update");
           publishBatteryVoltage();
       }
   }
  ...
  if (lastMinute != -1 && currentMinute != lastMinute) {
      ...
      if (currentHour == 0 && currentMinute == 0) {
          if (isDetailedLoggingEnabled()) Serial.println("Midnight! Refreshing page...");
          displayWeatherDashboard(false);
      } else {

          if (config.ui_mode == 0 && currentForecastCount > 0) {
              if (isDetailedLoggingEnabled()) Serial.println("Minute changed, updating display (Partial)...");
